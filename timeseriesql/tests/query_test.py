...
class TestQuery(unittest.TestCase):
    ...
    def test_gather_period(self):
        ...
        t1_timeindex = np.array([np.float64(t1.time[0] + x) for x in range(0, 3600, Query().DEFAULT_RESOLUTION)], dtype=np.float64)
        t2 = Query(x.mean for x in "test")[3600:1800]
        t2_timeindex = np.array([np.float64(t2.time[0] + x) for x in range(0, 1800, Query().DEFAULT_RESOLUTION)], dtype=np.float64)
        t3 = Query(x.mean for x in "test")[3600:1800:300]
        t3_timeindex = np.array([np.float64(t3.time[0] + x) for x in range(0, 1800, 300)], dtype=np.float64)
        t4 = Query(x.mean for x in "test").fetch()
        t4_timeindex = np.array([np.float64(t4.time[0] + x) for x in range(0, 3600, Query().DEFAULT_RESOLUTION)], dtype=np.float64)
